...
import org.eclipse.bpel.ui.editparts.policies.ColorChangePolicy;
...
//			}			
	...
	protected void createEditPolicies() {
		...
		//installEditPolicy(EditPolicy.SELECTION_FEEDBACK_ROLE, new ColorChangePolicy());
	}
...
//		this.getFigure().setBackgroundColor(new Color(null,0,255,255));
		System.out.println("activity refreshvisual");
		//C。从模型中取出颜色，如果模型中的颜色不是白色，则在边框变色，且running变红色。相当于控制器
		RGB col = ((ActivityImpl)this.getModel()).getColor();
		if(!col.equals(new RGB(255,255,255)))
		{	
			GroupBoxBorder b = new GroupBoxBorder();
			b.setLabel("Running");
			b.setTextColor(ColorConstants.red);
			this.getFigure().setBorder(b);//getfigure：得到视图V
			this.getFigure().setOpaque(true);
			this.getFigure().setForegroundColor(new Color(null,((ActivityImpl)this.getModel()).getColor()));//从模型中取出
			this.getFigure().setBackgroundColor(new Color(null,((ActivityImpl)this.getModel()).getColor()));
		}
		else//如果是白色，则边框去掉。
		{
			this.getFigure().setBorder(null);
			this.getFigure().setForegroundColor(new Color(null,((ActivityImpl)this.getModel()).getColor()));
			this.getFigure().setBackgroundColor(new Color(null,((ActivityImpl)this.getModel()).getColor()));
		}

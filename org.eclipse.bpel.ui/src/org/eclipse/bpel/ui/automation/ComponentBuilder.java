...
public class ComponentBuilder {
	...
	public ServiceInfo buildserviceinformation(ServiceInfo serviceinfo)
		...
		Definition def=reader.readWSDL(null, serviceinfo.getWsdllocation());//wsdl:definitions Definition对应这个标签，包含了下面的内容
		wsdlTypes = createSchemaFromTypes(def);//wsdl中的types  <wsdl:types>
		System.out.println("WSDL的Schema已经成功构建完,总共有多少个Schema定义:"+wsdlTypes.size());
		Map ns = def.getNamespaces();//得到wsdl的工作空间
		
		Set ss = ns.entrySet();
	Iterator iter = ss.iterator();//迭代器
	while(iter.hasNext())
	{
		...
		if (services!= null) {
			...
			populateComponent(serviceinfo, (Service) svcIter.next());//得到name和operation
...
//				}
	...
	private ServiceInfo populateComponent(ServiceInfo component, Service service) {//将service标签中的内容填充到component里
		System.out.println("***************************");
		System.out.println("");
		...
		String name = qName.getLocalPart();//service的name
		System.out.println("name为:"+name);
		component.setName(name);
		component.setTargetnamespace(namespace);//放入namespace
		Map ports=service.getPorts();//得到port
		Iterator portIter = ports.values().iterator();
		while (portIter.hasNext()) {
			Port port = (Port) portIter.next();
			Binding binding = port.getBinding();//port中对应的binding中含有operation的定义和参数
			List operations=buildOperations(binding);//构建方法
			...
			while (operIter.hasNext()) {
				...
				component.addOperation(operation);//将方法放入component中
			}
		}
		return component;//主要包括name和operation、namespace等等
	}

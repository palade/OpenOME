...
public class SetEvaluationLabelAction extends AbstractActionHandler {
	...
	private void highLight(IStructuredSelection selection) {
		...
		if (evalField == EvaluationLabel.SATISFIED) {
			color = "green";
		} else if (evalField == EvaluationLabel.DENIED) {
			color = "red";
		} else if (evalField == EvaluationLabel.PARTIALLY_SATISFIED) {
			color = "blue";
		} else if (evalField == EvaluationLabel.CONFLICT
				|| evalField == EvaluationLabel.UNKNOWN) {
			color = "yellow";
		} else if (evalField == EvaluationLabel.PARTIALLY_DENIED) {
			color = "orange";
		}
		HighlightIntentionsCommand highlight = new HighlightIntentionsCommand(
				selection.toList(), os, color);

		highlight.execute();
	}

	public void applyEvalLabel(final EObject object,
			IProgressMonitor progressMonitor, DiagramCommandStack dcs) {
		MyCommand applyLabel = new MyCommand(object) {
			...
			protected CommandResult doExecuteWithResult(
				...
				if (object instanceof Intention) {
					((Intention) object)
							.setQualitativeReasoningCombinedLabel(evalField);
				}
	...
	private abstract class MyCommand extends AbstractTransactionalCommand {
		public MyCommand(EObject elt) {
			super((TransactionalEditingDomain) AdapterFactoryEditingDomain
					.getEditingDomainFor(elt), evalLabel,

...
/**
 * @author jenhork This is a child of the Reasoner class, a specific type of
 *         reasoner. Specifically, it implements qualitative, interactive
 *         reasoning.
 * 
 */
public class InteractiveQualReasoner extends Reasoner {
	...
	// Some extra info needs to be stored with softgoals, so we have a wrapper
	// class, and we have another class
	// which stores a list of these wrappers, this is that class
	private Vector<Intention> softgoalWrappers;
	// Hard intentions (non-softgoals: goal, task, resource) which have been in
	// each iteration of the algorithm.
	// We keep track of this to avoid resolving one multiple times in an
	// iteration
	...
	/**
	 * @author jenhork Constructor, takes in a ModelImpl (how the model is
	 *         stored) a CommandStack, to execute commands, also a diagram
	 *         Command stack
	 */
	public InteractiveQualReasoner(ModelImpl m, CommandStack com,
			DiagramCommandStack d, List editParts) {
	 ...
	 */
	public InteractiveQualReasoner(ModelImpl m, CommandStack com,
			DiagramCommandStack d, List editParts, boolean autoEvaluate) {
		this(m, com, d, editParts);
		this.autoEvaluate = autoEvaluate;

	}

	/**
	 * @author jenhork The major method for qualitative, interactive reasoning.
	 *         This is like the "main" method. Where most of the stuff starts
	 *         from.
	 */
	public void reason() {
		...
		// Initialize the label queue (obviously). This grabs all the labels on
		// the model and puts them in the queue
		initLabelQueue();
		...
		while (lq.size() > 0) {
			...
			// do step one of the algorithm, this is where all the labels are
			// propagated one step and labels are added
			// to label bags
			step1();

			resolvedHardIntentions.clear();

			// DEBUGGING, print out the list of softgoals which have label bags
			// that need resolving
			...
			// do step two. If not step 2, i.e. the user has pressed cancel,
			// stop the entire algorithm
			if (!step2()) {
				timer.stopReasoningTimer();
				return;
			}

			// DEBUGGING, at the end of each iteration, print out the label
			// queue
			System.out.println("Label Queue");
			lq.print();
	...
	/**
	 * @author jenhork This looks through all the intentions and finds the ones
	 *         with evaluation labels. They must be propagated so the intention
	 *         is added to the label queue.
	 */
	private void initLabelQueue() {
		...
		for (Intention i : model.getAllIntentions()) {
			...
			if (initvalue != EvaluationLabel.NONE) {
				...
				// store the initial value separately from the actual value of
				// the intention
				Command setInitLabel = new SetInitialEvaluationLabelCommand(i,
						initvalue);
				cs.execute(setInitLabel);
				highlightIntention(i, initvalue);
				// System.out.println("initial value set");
				// if the intention is a softgoal, add the initial value to the
				// label bag for a softgoal
				if (i instanceof Softgoal) {
					try {
						// For initial values, the source element in the label
						// bag is the element itself
						Command addToLB = new AddToLabelBagCommand(i, i,
								initvalue);
						cs.execute(addToLB);
					} catch (Exception e) {
				...
				cs.execute(setInitLabel);
				highlightIntention(i, EvaluationLabel.NONE);
			}
		}
	}

	/**
	 * @author jenhork This method propagates everything in the label queue at
	 *         this time, including adding labels to softgoal bags.
	 */
	private void step1() {
		// DEBUGGING
		...
		for (int i = 0; i < starting_size; i++) {
			...
			// try to grab the right element from the queue and put it in the
			// wrapper
			...
			// propagate through the decomposition and means-ends links from
			// this element
	...
	/**
	 * @author jenhork Takes an intention which has a label, as it comes from
	 *         the label queue, and propagates the current label through all of
	 *         the contributions links coming from that intention, i.e. with
	 *         that intention as a source.
	 */
	private void propagateContributions(Intention intention) {
		// DEBUGGING
		System.out
				.println("propagate Contributions for " + intention.getName());

		// Get all the intentions that this intention contributes to via
		// contribution links
		for (Contribution c : intention.getContributesTo()) {
			...
			EvaluationLabel result = applyContributionRules(c,
					intention.getQualitativeReasoningCombinedLabel());

			// DEBUGGING
			System.out.println("Result: " + result.getName());

			// Add the softgoal to a list of softgoals to resolve via human
			// judgment
	...
	/**
	 * @author jenhork Add the softgoal to a list of softgoals to resolve via
	 *         human judgment
	 */
	private void addSoftgoalToResolve(Intention target, Intention source,
	...
	/**
	 * @author jenhork This implements the contribution link table in my
	 *         evaluation work. See table 3 here:
	 *         http://istar.rwth-aachen.de/tiki-index.php?page_ref_id=260
	 * 
	 */
	private EvaluationLabel applyContributionRules(Contribution c,
			EvaluationLabel l) {
		...
		if (c instanceof HelpContribution) {
			if (l == EvaluationLabel.SATISFIED
					|| l == EvaluationLabel.PARTIALLY_SATISFIED)
				return EvaluationLabel.PARTIALLY_SATISFIED;
			if (l == EvaluationLabel.DENIED
					|| l == EvaluationLabel.PARTIALLY_DENIED) {
	...
	/**
	 * @author jenhork Propagates values through decompositions and means-ends
	 *         links coming from this intention, i.e., the intention is the
	 *         child of the relationships
	 */
	private void propagateDecompositions(Intention intention) {
		// DEBUGGING
		System.out.println("Propagating Decompositions for "
				+ intention.getName());

		// For each decomposition link coming "from" an intention (i.e.) the
		// intention is the child
		for (Decomposition d : intention.getDecompositionsFrom()) {
			// get the target or parent
			Intention target = d.getTarget();
			// If the target has not already been resolve in this iteration,
			// resolve it
			if (!resolvedHardIntentions.contains(target)) {
				// DEBUGGING
				System.out.println("Finding label for " + target.getName());

				// add the target to the list of hard intentions that have been
				// resolved
				resolvedHardIntentions.add(target);

				// Now actually find the value for the target. This has to be
				// mixed with dependencies to account
				// for intentions which have a mixture of decompositions and
				// dependencies
	...
	/**
	 * @author jenhork Propagates values through dependency links coming from
	 *         this intention, i.e., the intention is the source or dependee
	 * 
	 */
	private void propagateDependencies(Intention intention) {
		// DEBUGGING
		// System.out.println("Propagating Dependencies for " +
		// intention.getName());
		...
		for (Dependency d : intention.getDependencyTo()) {
			...
			if (!(dependable instanceof Container)) {
				// if the target has not already been resolved in this round of
				// propagation
				if (!resolvedHardIntentions.contains(dependable)) {
					// DEBUGGING
					// System.out.println("Finding label for " + ((Intention)
					// dependable).getName());

					// Now actually find the value for the target. This has to
					// be mixed with dependencies to account
					// for intentions which have a mixture of decompositions and
					// dependencies
					resolveDecompositionsAndDependencies(
							(Intention) dependable, intention);

					// Add the intention to the list of hard intentions resolved
					// in this round
	...
	/**
	 * @author jenhork Actually find a value for a hard intention. We need to
	 *         take into accound decompositions, means-ends and dependencies, so
	 *         we look at them all at once in one method.
	 */
	private void resolveDecompositionsAndDependencies(Intention target,
		...
		// Go through all of the outgoing decomposition links and gather in
		// incoming values
		// So it finds all of the children of the target via decomposition or
		// means-ends links
		// and grabs their labels
		for (Decomposition dc : target.getDecompositionsTo()) {
			if (dc instanceof AndDecomposition) {
				ANDDecomps.add(dc.getSource()
						.getQualitativeReasoningCombinedLabel());
			}
			if (dc instanceof OrDecomposition) {
				ORDecomps.add(dc.getSource()
						.getQualitativeReasoningCombinedLabel());
			}
		}
		...
		// Do the same for dependencies, find all the incoming labels via
		// dependencies
		...
		for (Dependency dep : target.getDependencyFrom()) {
			...
			// Make sure it's not coming from an actor, if it is, it won't have
			// a label
			if (!(dependable instanceof Container)) {
				result = ((Intention) dependable)
						.getQualitativeReasoningCombinedLabel();
				dependencies.add(result);

				// If the intention we are resolving is a softgoal, add the
				// values from dependency links
		...
		System.out.println(ANDDecomps.size() + ", " + ORDecomps.size() + ", "
				+ dependencies.size());

		// An element probably shouldn't have both an AND and an OR
		// Decomposition, doesn't
		// make much sense, but, in case it does, I will do something. I will
		// AND the results
		...
		// if the target is a softgoal, add the AND of the AND and OR results to
		// it's label bag, if it's not none
		if (target instanceof Softgoal) {
			if (both.size() > 0) {
		...
		// target is not a softgoal
			...
			// there are dependencies, so add in the dependency results to the
			// both variable (which should now be called
			// "all three" I guess)
			if (dependencies.size() > 0) {
				both.add(resolveAND(dependencies));
			}

			// Now AND together the results of AND, OR and dependency, this is
			// the final answer
			result = resolveAND(both);
			...
			if (result != EvaluationLabel.NONE) {
				...
				highlightIntention(target,result);
	...
	/**
	 * @author jenhork Takes in a vector of evaluation labels and "ANDs" them,
	 *         finds the smallest and returns it. See
	 *         http://istar.rwth-aachen.de/tiki-index.php?page_ref_id=260 for
	...
	private EvaluationLabel resolveAND(Vector<EvaluationLabel> list) {
		...
		for (EvaluationLabel l : list) {
			// DEBUGGING, System.out.println("Is " + l.getName() + " less than "
			// + smallest.getName() + "?");
	...
	/**
	 * @author jenhork Takes in a vector of evaluation labels and "ORs" them,
	 *         finds the largest and returns it. See
	 *         http://istar.rwth-aachen.de/tiki-index.php?page_ref_id=260 for
	 *         the ordering of labels.
	 */
	private EvaluationLabel resolveOR(Vector<EvaluationLabel> list) {
		...
		for (EvaluationLabel l : list) {
			// DEBUGGING, System.out.println("Is " + l.getName() +
			// " greater than " + biggest.getName() + "?");
	...
	/**
	 * @author jenhork Step 2! In step two we go through all the unresolved
	 *         softgoals and call for human judgment on them. The results are
	 *         taken and added into the label queue for the next iteration
	 */
	private boolean step2() {
		...
		for (Intention i : softgoalWrappers) {
			...
			if (i.getLabelBag().needResolve()) {
				...
				if (result != null)
					System.out.println("Automatic result: " + result.getName());
				...
				if (result == null) {
					...
					if (result == null
							&& i.getInitialEvalLabel() == EvaluationLabel.NONE) {
						// get human judgment
						timer.startHumanJudgmentTimer();
						result = humanJudgement(i);
						timer.stopHumanJudgmentTimer();

						// the result will be null if they cancel the window,
						// which means they want to quit evaluating
						if (result == null) {
							return false;
						}
				...
				if (i.getInitialEvalLabel() == EvaluationLabel.NONE) {
					setQualCombinedLabel(i, result);
					highlightIntention(i, result);
	...
	/**
	 * @author jenhork Apply the cases where softgoal label bags can be resolved
	 *         automatically. See Table 4 here for a list:
	 *         http://istar.rwth-aachen.de/tiki-index.php?page_ref_id=260
	 */
	private EvaluationLabel applyAutomaticSoftgoalCases(Intention i) {
		...
		// case 2 & 3
		if (i.getLabelBag().hasFullPositive()
				&& i.getLabelBag().isAllPositive())
			return EvaluationLabel.SATISFIED;
		if (i.getLabelBag().hasFullNegative()
				&& i.getLabelBag().isAllNegative())
			return EvaluationLabel.DENIED;
		// new cases
	...
	protected EvaluationLabel resolveOtherCases(Intention i) {
		...
		ListIterator<EvaluationLabel> it = i.getLabelBag()
				.getLabelBagEvalLabels().listIterator();
		...
		// System.out.println(FSPerc + ", " + PSPerc + ", " + CPerc + ", " +
		// UPerc + ", " + PDPerc + ", " + FDPerc);
	 ...
	 * 
	 */
	protected EvaluationLabel humanJudgement(Intention i) {
		...
		HighlightIntentionOutlinesCommand highlightTarget = new HighlightIntentionOutlinesCommand(
				editParts, target, new RGB(255, 0, 0)); // 255 0 0 is rgb for
														// red for target
		HighlightIntentionOutlinesCommand highlightChildren = new HighlightIntentionOutlinesCommand(
				editParts, children, new RGB(0, 0, 255)); // 0 0 255 is rgb for
															// blue for children
		...
		System.out.println("Human judgment time for analysis was "
				+ (end - start) + " ms.");
	...
	private void highlightIntention(Intention i, EvaluationLabel label) {
		String color = "";
		if (label == EvaluationLabel.SATISFIED) {
			color = "green";

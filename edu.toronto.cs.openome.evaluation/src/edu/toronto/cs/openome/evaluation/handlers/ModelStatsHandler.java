...
public class ModelStatsHandler extends ReasonerHandler {
	...
	private void getModelStats() {
		...
		// Count Links
		types = "All,Contribution,Dependency,Means-Ends,Decomposition,Association"
				.split(",");
		...
		// This should be temporary, it would be better to have a nicer format
		// pop-up with text
		// is copyable. You can't copy text from this pop-up now, which is
		// annoying.
		Shell[] ar = PlatformUI.getWorkbench().getDisplay().getShells();
		Shell shell = ar[0];
		showMessage("Interactive Qualitative Backward Reasoning", message,
				shell);
